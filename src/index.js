...
async function authenticate(username, password) {
  ...
  return request
          ...
          return request
            ...
            .then(x => {
              if (x.statusCode === 302) {
                //catch redirect
                const redirectUrl =
                  x.rawHeaders[x.rawHeaders.indexOf('Location') + 1]
                return request
                  .get({
                    url: redirectUrl,
                    resolveWithFullResponse: true,
    ...
    .catch(e => log('warn', 'SSO pre-start error', e))
}
...
function parseActivities(start, limit) {
  return request
    ...
    .catch(e => {
      log('warn', 'list was not collected', e)
      return null
    })
}
...
function parseActivityDetails(activityId) {
  return request
    .get({
      url:
        baseUrl + 'proxy/activity-service/activity/' + activityId + '/details',
      jar: j
    })
    .then(x => {
      return x
